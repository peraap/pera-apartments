      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center">
          ...
          <div className="hidden md:flex items-center space-x-10">
            <Link 
              to="/"
              className={`text-xs font-bold uppercase tracking-[0.2em] transition-all duration-500 hover:opacity-50 relative after:content-[''] after:absolute after:bottom-[-4px] after:left-0 after:w-0 after:h-[1px] after:bg-current after:transition-all hover:after:w-full ${scrolled ? 'text-black' : 'text-white'}`}
            >
              {t.home}
            </Link>
            ...
            <div className="relative group/info" onMouseEnter={() => setIsInfoOpen(true)} onMouseLeave={() => setIsInfoOpen(false)}>
              <button className={`flex items-center text-xs font-bold uppercase tracking-[0.2em] transition-all duration-500 hover:opacity-50 ${scrolled ? 'text-black' : 'text-white'}`}>
                {t.info}
                <ChevronDown size={12} className={`ml-1 transition-transform duration-300 ${isInfoOpen ? 'rotate-180' : ''}`} />
              </button>
              <AnimatePresence>
                {isInfoOpen && (
                  <motion.div 
                  ...
                  >
                    {infoLinks.map((link) => (
                      <HashLink 
                        key={link.path}
                        to={link.path}
                        className="block px-6 py-3 text-xs font-bold uppercase tracking-widest text-black hover:bg-neutral-50 transition-colors"
                      >
                        {link.name}
                      </HashLink>
                    ))}
              ...
              {user ? (
                <div className="flex items-center space-x-3 cursor-pointer">
                  <div className="text-right hidden lg:block">
                    <p className={`text-[10px] font-black uppercase tracking-widest ${scrolled ? 'text-black' : 'text-white'}`}>
                ...
                <Link 
                  to="/login"
                  className={`flex items-center space-x-2 text-xs font-bold uppercase tracking-[0.2em] transition-all duration-500 hover:opacity-50 ${scrolled ? 'text-black' : 'text-white'}`}
                >
                  <User size={14} />
                  <span>Login</span>
                </Link>
                ...
                {isUserOpen && user && (
                  ...
                  >
                    <div className="px-6 py-4 border-b border-neutral-50 bg-neutral-50/50">
                      <p className="text-[10px] font-black uppercase tracking-widest text-neutral-400 mb-1">Puncte Loialitate</p>
                      <p className="text-lg font-black text-black">{profile?.points || 0} <span className="text-[10px] text-yellow-600">PTS</span></p>
                    </div>
                    {profile?.role === 'admin' && (
                      <Link 
                        to="/admin"
                        className="flex items-center space-x-3 px-6 py-3 text-xs font-bold uppercase tracking-widest text-black hover:bg-neutral-50 transition-colors"
                      >
                    ...
                    <button 
                      onClick={() => logout()}
                      className="w-full flex items-center space-x-3 px-6 py-3 text-xs font-bold uppercase tracking-widest text-red-600 hover:bg-red-50 transition-colors"
                    >
                      <X size={14} />
                      <span>Logout</span>
                    </button>
            ...
            <a 
              ...
              className={`px-8 py-2.5 rounded-full text-[10px] font-bold uppercase tracking-[0.2em] transition-all duration-500 ${scrolled ? 'bg-black text-white hover:bg-neutral-800 shadow-lg shadow-black/10' : 'bg-white text-black hover:bg-neutral-100'}`}
            >
              {t.reserve}
            </a>
          </div>
          ...
          <div className="md:hidden flex items-center space-x-4">
            <button 
              ...
              className={`text-[10px] font-bold uppercase tracking-widest ${scrolled ? 'text-black' : 'text-white'}`}
            >
              {lang === 'ro' ? 'EN' : 'RO'}
            </button>
            <button 
          ...
          >
            <div className="flex flex-col space-y-6 items-center">
              <Link 
                ...
                className="text-lg font-medium text-black uppercase tracking-widest"
              >
                {t.home}
              </Link>
              <div className="w-full border-t border-neutral-100 pt-6 flex flex-col items-center space-y-4">
                <span className="text-[10px] uppercase tracking-widest text-neutral-400 font-bold">{t.info}</span>
                {infoLinks.map((link) => (
                  <HashLink 
                    key={link.path}
                    to={link.path}
              ...
              <a 
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="w-full text-center bg-black text-white py-4 rounded-full font-bold uppercase tracking-widest"
              >
                {t.reserve}
              </a>
            </div>
    ...
    <footer className="bg-neutral-900 text-white pt-20 pb-10">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-12 mb-16">
          ...
          <div>
            <h4 className="text-xs uppercase tracking-[0.2em] font-bold mb-6 text-white">{t.navigation}</h4>
          ...
          <div>
            <h4 className="text-xs uppercase tracking-[0.2em] font-bold mb-6 text-white">{t.contact}</h4>
          ...
          <div>
            <h4 className="text-xs uppercase tracking-[0.2em] font-bold mb-6 text-white">{t.newsletter}</h4>
            <p className="text-xs text-white mb-4">{t.newsletterDesc}</p>
